...
# Сгенерировано веб-интерфейсом kb-web. Не редактируйте пути без необходимости.
#
...
services:
  {cname}:
    ...
    build:
      ...
      dockerfile: packages/kb/docker/Dockerfile
    image: {img}
    ...
    volumes:
      ...
      - {project_root / "profiles"}:/app/profiles{gpu_block}
"""
...
def compose_up(compose_dir: Path, config: ProfileConfig, *, rebuild: bool = False) -> str:
    ...
    if rebuild:
        args.append("--build")
    result = _run_compose(compose_dir, config.profile_name, *args, timeout=3600)
...
def compose_build(compose_dir: Path, config: ProfileConfig, *, no_cache: bool = False) -> str:
    write_compose_file(compose_dir, config)
    args = ["build"]
    if no_cache:
    ...
    output = ((result.stdout or "") + (result.stderr or "")).strip()
    if result.returncode != 0:
        raise RuntimeError(output or "docker compose build завершился с ошибкой")
    return output
