...
- «Какие регистры затрагивает объект Y?» → `get_object(..., detail="movements")`
- «Какие объекты входят в подсистему УправлениеЗапасами?» → `list_by_relation(relation="objects_in_subsystem", ...)`
- «Покажи BSL-модули документа Z» → `get_object(..., detail="full")` или `get_module`
...
| **mcp_server/** | Docker (или хост) | Только чтение Chroma + 5 MCP tools |
...
Сервер регистрирует **5 инструментов** (`packages/kb/mcp_server/server.py`):
...
| `get_module` | Чтение BSL: summary, procedure, event, fragment |
...
- «Покажи `get_object` с `detail="movements"` для Document.РеализацияТоваровУслуг»
...
| 4 | **5 MCP tools** | get_object(detail), list_by_relation, get_module, find_references(scope) |
...
│   └── …
├── mcp_server/server.py       # 5 MCP tools
├── web/                       # Flask UI :5050
