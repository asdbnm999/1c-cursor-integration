...
На странице профиля в блоке **«2. Docker MCP-сервер»**:

- **MCP-проект** — путь к каталогу compose (`~/DockerMCP/1c-kb-<profile>/` по умолчанию). Поле ввода и **Выбрать…** создают подпапку `1c-kb-<profile>` в выбранном родительском каталоге.
- **Лимит RAM** — для контейнера MCP (512–8192 MB, по умолчанию 1024). Меняется только при остановленном контейнере; применяется к следующему запуску.
...
| `list_by_relation` | Обратные связи (документы по регистру, объекты подсистемы и др.) |
| `get_module` | Чтение BSL-модуля (summary, procedure, event, fragment) |
