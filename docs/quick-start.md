...
В блоке **«2. Docker MCP-сервер»** проверьте путь **MCP-проект** (по умолчанию `~/DockerMCP/1c-kb-<имя>/`; при необходимости — **Выбрать…**).  
Лимит RAM контейнера можно поднять для больших баз (применится к следующему запуску). Нажмите **«Запустить MCP»** — сборка образа и запуск выполняются автоматически. Порт профиля: `8300 + N` (например `8302`).
...
2. Убедитесь, что все серверы включены: `searxng`, `1c-syntax-helper`, `1c-kb-<имя-профиля>` — зелёный индикатор, инструменты/ресурсы загружены.

![MCP-серверы в Cursor](images/quick-start/18-cursor-mcp-enabled.png)

3. На dashboard нажмите **«Проверить MCP»** — все ваши серверы должны отвечать `OK`.
4. Вернитесь на dashboard: мастер и карточки §1–§4 — **«Готово»**.
